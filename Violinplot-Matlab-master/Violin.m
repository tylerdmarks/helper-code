classdef Violin < handle
    ...
    properties (Dependent=true)
        ViolinColor % fill color of the violin area and data points
        ViolinAlpha % transparency of the violin area
        ...
        ShowBoxPlot % whether to show Box plot
        ShowWhiskers   % whether to show whiskers
    end
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            obj.DataAlpha = args.DataAlpha;
            obj.ShowBoxPlot = args.ShowBoxPlot;
        ...
        function set.DataAlpha(obj, alpha)
            obj.ScatterPlot.MarkerFaceAlpha = alpha;
        end
        
        function alpha = get.DataAlpha(obj)
            alpha = obj.ScatterPlot.MarkerFaceAlpha;
        end
        ...
        function set.ShowBoxPlot(obj, yesno)
            if ~isempty(obj.BoxPlot)
                if yesno
        ...
        function yesno = get.ShowBoxPlot(obj)
            if ~isempty(obj.BoxPlot)
    ...
    methods (Access=private)
        function results = checkInputs(obj, data, pos, varargin)
            ...
            p.addParameter('ShowBoxPlot', true, isscalarlogical);
            p.addParameter('ShowWhiskers', true, isscalarlogical);
            p.addParameter('DataAlpha', 0.3, isscalarnumber);
